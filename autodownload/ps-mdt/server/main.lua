...
function proxyTriggerEvent(sessionId, eventType, ...)

	local args = { ... }
	local targetFn = nil

	local function RegisterNetEvent(event, func)
	...
	local function SendNUIMessage(data)
		TriggerEvent('u-qb-devices:mdt:post', sessionId, data)
	end

	--
	-- taken from @ps-mdt/client/main.lua
	--
	...
	if targetFn ~= nil then
		targetFn(table.unpack(args))
		-- else
		-- 	TriggerClientEvent(eventType, table.unpack(args))
	end
...
RegisterNetEvent('u-qb-devices:mdt:triggerAction', function(sessionId, eventType, ...)
	...
	local function TriggerClientEvent(event, src, ...)
		local args = { ... }
		if src == source then
			proxyTriggerEvent(sessionId, event, table.unpack(args))
		end
		OriginalTriggerClientEvent(event, src, table.unpack(args))
	end

	local function TriggerLatentClientEvent(event, src, x, ...)
		TriggerClientEvent(event, src, ...)
	end

	--
	-- taken from @ps-mdt/server/main.lua
	--
	...
	if targetFn ~= nil then
		...
		TriggerEvent(eventType, table.unpack(args))
	end
