...
function AddItem(identifier, item, amount, slot, info, reason)
    ...
    if not updated then
		...
		-- <>
        if item:lower() == 'phone' then
            if not inventory[slot].info.serial_no then
                inventory[slot].info.serial_no = QBCore.Shared.RandomSerial(32)
            end
		-- if itemInfo.type == 'weapon' then
        elseif itemInfo.type == 'weapon' then
		-- </>
