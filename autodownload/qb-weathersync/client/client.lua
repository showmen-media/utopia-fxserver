...
CreateThread(function()
    ...
    while true do
        if not disable then
			...
			-- <Utopia> Fix issue with time sync
            -- if tick - (Config.RealTimeSync and 500 or 22) > tick then
            if GetGameTimer() - (Config.RealTimeSync and 500 or 22) > tick then
			-- </Utopia>
			...
			correctSecond = second % 60
			correctMinute = minute + math.floor(second / 60)
			correctHour = hour + math.floor(correctMinute / 60)
			correctMinute = correctMinute % 60
			correctHour = correctHour % 24
			NetworkOverrideClockTime(correctHour, correctMinute, correctSecond)
			-- </Utopia>
        else
            Wait(1000)
        end
