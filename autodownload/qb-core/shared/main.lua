...
-- <>
local ReadableIdCharset = {
	'A', 'B', 'E', 'H', 'I', 'K', 'M', 'N', 'O', 'P', 'T', 'X', 'Y', 'Z'
}

function QBShared.RandomReadableIdChars(length)
    if length <= 0 then return '' end
    return QBShared.RandomReadableIdChars(length - 1) .. ReadableIdCharset[math.random(1, #ReadableIdCharset)]
end
...
function QBShared.RandomSerial(length)
    if length <= 0 then return '' end
    return QBShared.RandomSerial(length - 1) .. SerialCharset[math.random(1, #SerialCharset)]
end
-- </>
