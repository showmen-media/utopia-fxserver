...
-- <>
local huntingBlips = nil

local checkingBlips = false
local function checkHuntingBlips()
	if checkingBlips then return end
	checkingBlips = true

	Citizen.SetTimeout(1000, function()
		local hasBait = exports['qb-inventory']:HasItem('huntingbait', 1)
		if huntingBlips == nil then
			if hasBait then
				huntingBlips = createCustomBlips(Config.HuntingArea)
			end
		else
			if not hasBait then
				for _, v in pairs(huntingBlips) do
					RemoveBlip(v)
				end
				huntingBlips = nil
			end
